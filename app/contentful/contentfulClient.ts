...
import { ContentfulBlogPost, ContentfulMicroblog } from "./types";

let GLOBAL_CONTENTFUL_CLIENT: ContentfulClientApi<undefined> | null = null;
...
export const getContenfulMicroblogs = async () => {
  const queryResults = await getGlobalContentfulClient().getEntries({
    content_type: "microblogs",
    ...QUERY_ONLY_VISIBLE_IN_PRODUCTION,
    order: ["-sys.updatedAt"],
  });
  return queryResults as EntryCollection<ContentfulMicroblog>;
};
