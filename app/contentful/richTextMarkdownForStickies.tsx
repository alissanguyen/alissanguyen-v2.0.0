...
export const stickyOptions: Options = {
  ...
  renderNode: {
    [INLINES.HYPERLINK]: (node: Node, children) => {
      ...
      return (
        <a
          className={`BlogPost__HyperLink ${theme === SupportedTheme.DARK
    ...
    [BLOCKS.LIST_ITEM]: (node: Node | unknown, children) => {
      return (
        <li className="list-item text-lg leading-8 ml-10">{children}</li>
      );
    },
    [BLOCKS.HR]: () => <div className="spacer-div h-7"></div>,
    ...
    [BLOCKS.EMBEDDED_ASSET]: (node) => {
      ...
      const maybeDescription = node.data.target.fields.description as
        | string
        | undefined;
      // TODO: Stickies should only take images as media assets, no videos
      return (
        <ImageMedia
