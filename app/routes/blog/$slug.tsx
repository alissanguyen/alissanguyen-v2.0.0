...
export const meta: MetaFunction<typeof loader> = ({ data, location }) => {
  try {
     ...
     */
    const emptyMeta: MetaDescriptor = {};
    return emptyMeta;
  }
};
      ...
      <div className="text-post-bodyText">
        ...
        >
          ...
          <h1 className="BlogPost__Title flex w-full text-4xl text-post-bodyTextLg xs:text-5xl font-bold max-w-[700px] mx-auto">
            {blogPost.fields.blogPostTitle as string}
          </h1>
          ...
          <div className="flex flex-col lg:flex-row lg:justify-between my-16">
            <div className="text-base mb-16 lg:mb-0">
              ...
              {blogPost.metadata.tags.map((tag: any) => (
                <TagBadge tag={tag} theme={theme} key={tag}/>
              ))}
            </div>
          </div>
          <AuthorSection />
        ...
        <hr></hr>
        {blogPostWithAtLeastOneSharedTag.length > 0 ? <RelatedPostsSection relatedPosts={blogPostWithAtLeastOneSharedTag} /> : null}
        <hr></hr>
      </div>
    </>
  );
